...
QualityEstimator::SentenceQualityEstimate QualityEstimator::mapBPEToWords(Response& sentence, Words words) const {
  ...
  for (int sentenceIdx = 0; sentenceIdx < sentence.size(); sentenceIdx++) {
    ...
    // first_score
    sentenceQualityScores.wordByteRanges.push_back(sentence.target.wordAsByteRange(sentenceIdx, 0));
    sentenceQualityScores.wordQualityScores.push_back(quality.word.front());
    features.bpeMean = quality.word.front();
    features.lenBPE = 1;
    features.minBPE = quality.word.front();
    features.overallMean = quality.sequence / quality.word.size();
    sentenceQualityScores.wordFeatures.push_back(features);
    // first_score
    int wordIdx = 1;
    int num_subwords = 1;
    for (int i = 1; i < quality.word.size(); i++) {
      ...
      if (isspace(first_word) != 0) {
        ...
        auto& current_features = sentenceQualityScores.wordFeatures.back();
        if (current_features.minBPE > p) {
          current_features.minBPE = p;
        }
        num_subwords += 1;
        current_features.lenBPE = num_subwords;
        ByteRange new_word{current_word.begin, subword.end};
        current_word = new_word;
        // incremental mean
        current_scores = current_scores + (p - current_scores) / num_subwords;
        current_features.bpeMean = current_scores;
      }
    }
...
void QualityEstimator::computeQualityScores(Response& sentence, Words words) const {
  auto qualityScores = mapBPEToWords(sentence, words);
