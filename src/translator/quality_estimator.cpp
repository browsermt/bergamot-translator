...
void UnsupervisedQualityEstimator::computeQualityScores(const ProcessedRequestSentences& processedRequestSentences,
                                                        Response& response) const {
  for (size_t i = 0; i < processedRequestSentences.size(); ++i) {
    response.qualityScores.push_back(
        std::move(computeSentenceScores(processedRequestSentences[i].wordScores(), response.target, i)));
  }
}

Response::SentenceQualityScore UnsupervisedQualityEstimator::computeSentenceScores(
    const ProcessedRequestSentence::WordScores& logProbs, const AnnotatedText& target, const size_t sentenceIdx) const {
  ...
  for (const SubwordRange& wordIndice : wordIndices) {
    wordScores.push_back(
        std::accumulate(logProbs.cbegin() + wordIndice.begin, logProbs.cbegin() + wordIndice.end, float(0.0)) /
        wordIndice.size());
  }
...
void LogisticRegressorQualityEstimator::computeQualityScores(const ProcessedRequestSentences& processedRequestSentences,
                                                             Response& response) const {
  for (size_t i = 0; i < processedRequestSentences.size(); ++i) {
    response.qualityScores.push_back(
        std::move(computeSentenceScores(processedRequestSentences[i].wordScores(), response.target, i)));
...
LogisticRegressorQualityEstimator::Matrix LogisticRegressorQualityEstimator::extractFeatures(
    const std::vector<SubwordRange>& wordIndices, const ProcessedRequestSentence::WordScores& logProbs) const {
...
std::vector<SubwordRange> mapWords(const ProcessedRequestSentence::WordScores& logProbs, const AnnotatedText& target,
