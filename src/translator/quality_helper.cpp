...
AlignedMemory QualityHelper::writeQualityEstimatorMemory( const std::vector< std::vector< float > >& logisticRegressorParameters )
{
    ...
    marian::bergamot::AlignedMemory memory(sizeof(header) + logisticRegressorParameters.size() * parametersDims * sizeof(float));
