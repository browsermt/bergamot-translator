...
var Module = {
  ...
  onRuntimeInitialized: function() {
    log(`Wasm Runtime initialized Successfully (preRun -> onRuntimeInitialized) in ${
        (Date.now() - moduleLoadStart) / 1000} secs`);
...
onmessage =
  ...
  if (command === 'import') {
    ...
    try {
      ...
      log(`Translation '${from}${to}' Successful. Speed: ${Math.round(inputWordCount / secs)} WPS (${
          inputWordCount} words in ${secs} secs)`);
    } catch (error) {
...
const constructTranslationService =
    async () => {
...
const constructTranslationModel =
    async (from, to) => {
  ...
  if (from !== 'en' && to !== 'en') {
    ...
    await Promise.all(
        [_constructTranslationModelInvolvingEnglish(from, 'en'), _constructTranslationModelInvolvingEnglish('en', to)]);
  } else {
...
const _prepareAlignedMemoryFromBuffer =
    async (buffer, alignmentSize) => {
...
const _constructTranslationModelInvolvingEnglish =
    async (from, to) => {
...
`;
  ...
  const downloadedBuffers =
      await Promise.all([_downloadAsArrayBuffer(modelFile), _downloadAsArrayBuffer(shortlistFile)]);
  ...
  let constructedAlignedMemories = await Promise.all(
      [_prepareAlignedMemoryFromBuffer(modelBuffer, 256), _prepareAlignedMemoryFromBuffer(shortListBuffer, 64)]);
  ...
  var translationModel =
      new Module.TranslationModel(modelConfig, alignedModelMemory, alignedShortlistMemory, alignedVocabsMemoryList);
...
const _parseTranslatedTextSentences =
    (vectorResponse) => {
      const result = [];
      for (let i = 0; i < vectorResponse.size(); i++) {
        const response = vectorResponse.get(i);
...
const _parseSourceTextSentences =
    (vectorResponse) => {
      const result = [];
      for (let i = 0; i < vectorResponse.size(); i++) {
        const response = vectorResponse.get(i);
...
const _parseTranslatedTextSentenceQualityScores =
    (vectorResponse) => {
      const result = [];
      for (let i = 0; i < vectorResponse.size(); i++) {
        const response = vectorResponse.get(i);
